...
//
// Shows: large fan chart, final-value histogram, P10/50/90 cards
// ─────────────────────────────────────────────────────────────
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <Card>
        ...
        <div style={{ fontSize: 11, color: C.dim, marginBottom: 16 }}>
          Histogram of ending balance at year {years} across all {n.toLocaleString()} runs
        </div>
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={histogram} margin={{ top: 5, right: 16, bottom: 30, left: 8 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#0d2040" vertical={false} />
            <XAxis dataKey="label" tick={{ fill: C.dim, fontSize: 9, fontFamily: "JetBrains Mono, monospace" }} angle={-45} textAnchor="end" interval={2} />
            <YAxis tick={{ fill: C.dim, fontSize: 10 }} />
            <Tooltip content={<BarTip />} />
            <Bar dataKey="count" name="Simulations" radius={[3, 3, 0, 0]}>
              {histogram.map((_, i) => (
                <Cell key={i} fill={`hsl(${210 + i * 3.5},72%,${26 + i * 1.2}%)`} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
